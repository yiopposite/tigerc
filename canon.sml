...
struct
  ...
  fun ICE msg = ErrorMsg.impossible ("CANON: " ^ msg)

  fun linearize(stm0: T.stm) : T.stm list = let
      infix %
	...
	| do_stm(T.JUMP(e,labs)) =
	  let val (stms, [e']) = reorder [e]
	  in stms % T.JUMP(e', labs) end

	| do_stm(T.CJUMP(p,a,b,t,f)) = 
	  let val (stms, [a', b']) = reorder [a, b]
	  in stms % T.CJUMP(p, a', b', t, f) end

	| do_stm(T.MOVE(T.TEMP t,T.CALL(e,el))) = 
	  let val (stms, e'::el') = reorder (e::el)
	  in stms % T.MOVE(T.TEMP t, T.CALL(e', el')) end

	| do_stm(T.MOVE(T.TEMP t,b)) = 
	  let val (stms, [b']) = reorder [b]
	  in stms % T.MOVE(T.TEMP t, b') end

	| do_stm(T.MOVE(T.MEM e,b)) = 
	  let val (stms, [e', b']) = reorder [e, b]
	  in stms % T.MOVE(T.MEM e', b') end
	...
	| do_stm s = s
	  ...
	  let val (stms, [a', b']) = reorder [a, b]
	  in (stms, T.BINOP(p, a', b')) end

	| do_exp(T.MEM(a)) = 
	  let val (stms, [a']) = reorder [a]
	  in (stms, T.MEM a') end
